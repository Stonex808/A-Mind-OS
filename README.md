...
- A local activity log and memory panel that prefers real orchestrator output from `ui/shell/orchestrator-feed.json` (generated by `scripts/dev.sh`) and falls back to `ui/shell/demo-data.json` when no feed exists.
