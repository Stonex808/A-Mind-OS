...
[[ -f "$MEMORY_DIR/semantic_local/facts.json" ]] || fail "missing semantic memory facts.json"
[[ -f "$MEMORY_DIR/semantic_local/concepts.json" ]] || fail "missing semantic memory concepts.json"
[[ -d "$MEMORY_DIR/episodic_local/episodes" ]] || fail "missing episodic memory episodes directory"
[[ -d "$MEMORY_DIR/procedural" ]] || fail "missing procedural memory directory"
...
with sqlite3.connect(db_path) as conn:
    count = conn.execute("SELECT COUNT(*) FROM orchestrator_runs").fetchone()[0]
    accepted = conn.execute("SELECT COUNT(*) FROM orchestrator_runs WHERE accepted = 1").fetchone()[0]
if count < 2:
    raise SystemExit(f"expected at least 2 rows in {db_path}, found {count}")
if accepted < 2:
    raise SystemExit(f"expected at least 2 accepted rows in {db_path}, found {accepted}")
